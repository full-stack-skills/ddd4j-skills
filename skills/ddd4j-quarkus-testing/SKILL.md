...
description: Use when testing ddd4j-quarkus with QuarkusTest, QuarkusTestResource, Arc validation, Docker conditions, Testcontainers, classloader boundaries, and native tests.
license: Apache-2.0
---
...
统一介绍 QuarkusTest、TestResource、Arc、Docker、Testcontainers、ClassLoader、native test，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。
...
- “使用 `$ddd4j-quarkus-testing` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-testing` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-testing` 给出实现选择、证据状态和剩余风险。”
