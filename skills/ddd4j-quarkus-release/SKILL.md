...
description: Use when validating, checking CI, publishing, or remotely consuming ddd4j-quarkus 3.3.x and 4.0.x with Maven 3 or 4, security status, module inventory, and private repository contracts.
license: Apache-2.0
---
...
统一介绍 双线 clean build、Git SHA、CI、Security、私服、61模块、空缓存消费，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。

## 核心范围

双线 clean build、Git SHA、CI、Security、私服、61模块、空缓存消费。
...
- “使用 `$ddd4j-quarkus-release` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-release` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-release` 给出实现选择、证据状态和剩余风险。”
