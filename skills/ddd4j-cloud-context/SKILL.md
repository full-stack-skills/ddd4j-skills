...
统一介绍 请求头、ThreadContext、异步线程、Reactor、Feign、恢复与清理，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。

## 核心范围

请求头、ThreadContext、异步线程、Reactor、Feign、恢复与清理。
...
- “使用 `$ddd4j-cloud-context` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-context` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-context` 给出实现选择、证据状态和剩余风险。”
