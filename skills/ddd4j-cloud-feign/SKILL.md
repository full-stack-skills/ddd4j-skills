...
description: Use when configuring ddd4j-cloud Feign interceptors, context and tenant headers, error decoder factories, retries, request cleanup, and service-to-service contracts.
license: Apache-2.0
---
...
统一介绍 request interceptor、context/tenant header、error decoder、retry、cleanup，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。
...
- “使用 `$ddd4j-cloud-feign` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-feign` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-feign` 给出实现选择、证据状态和剩余风险。”
