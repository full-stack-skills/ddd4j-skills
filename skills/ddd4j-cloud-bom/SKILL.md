...
description: Use when managing ddd4j-cloud parent, dependencies, BOM imports, Spring Cloud and Alibaba versions, Boot parent alignment, effective POM, or consumer coordinates.
license: Apache-2.0
---
...
统一介绍 Cloud/Alibaba BOM、Boot parent、ddd4j BOM、版本所有权、effective POM，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。

## 核心范围

Cloud/Alibaba BOM、Boot parent、ddd4j BOM、版本所有权、effective POM。
...
- “使用 `$ddd4j-cloud-bom` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-bom` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-bom` 给出实现选择、证据状态和剩余风险。”
