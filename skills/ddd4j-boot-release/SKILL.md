...
统一介绍 13 线 clean build、Git SHA、CI、Security、私服 deploy、空缓存消费，不按单个 artifact 拆技能。使用前先通过 ddd4j-boot-version-selection 确认维护线。

## 核心范围

13 线 clean build、Git SHA、CI、Security、私服 deploy、空缓存消费。
...
- “使用 `$ddd4j-boot-release` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-boot-release` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-boot-release` 给出实现选择、证据状态和剩余风险。”
