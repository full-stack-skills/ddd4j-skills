...
description: Use when integrating JWT, OIDC, Shiro, or Sa-Token authentication with ddd4j-quarkus Subject providers, roles, permissions, request scope, and errors.
license: Apache-2.0
---
...
统一介绍 JWT、OIDC、Shiro、Sa-Token、SubjectProvider、RolesAllowed、request scope，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。
...
- “使用 `$ddd4j-quarkus-auth` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-auth` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-auth` 给出实现选择、证据状态和剩余风险。”
