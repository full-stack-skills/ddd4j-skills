...
description: Use when selecting or wiring local and distributed ddd4j caches in Javalin, including Caffeine, shared CAS, idempotency, TTL, lifecycle, and production validation.
license: Apache-2.0
---
...
统一介绍 Caffeine、本地缓存、分布式 Cache、shared CAS、TTL、幂等。先用 ddd4j-javalin-version-selection 确认维护线，再读取当前分支源码。

## 核心范围

Caffeine、本地缓存、分布式 Cache、shared CAS、TTL、幂等。
...
- “使用 `$ddd4j-javalin-cache` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-javalin-cache` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-javalin-cache` 给出实现选择、证据状态和剩余风险。”
