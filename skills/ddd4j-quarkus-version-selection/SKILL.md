...
description: Use when choosing a ddd4j-quarkus 3.3.x or 4.0.x line based on ddd4j, Quarkus Platform, JDK, Maven, and POM model constraints.
license: Apache-2.0
---
...
统一介绍 3.3.x→ddd4j2/JDK17/Maven3/Quarkus3.37.4；4.0.x→ddd4j3/JDK21/Maven4/Quarkus3.38.2，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。

## 核心范围

3.3.x→ddd4j2/JDK17/Maven3/Quarkus3.37.4；4.0.x→ddd4j3/JDK21/Maven4/Quarkus3.38.2。
...
- “使用 `$ddd4j-quarkus-version-selection` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-version-selection` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-version-selection` 给出实现选择、证据状态和剩余风险。”
