...
description: Use when configuring or reviewing ddd4j-cloud Nacos, Sentinel, monitoring, tracing, metrics, logging, service health, or readiness behavior.
license: Apache-2.0
---
...
统一介绍 Nacos、Sentinel、monitor、trace、metrics、logging、health、readiness，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。
...
- “使用 `$ddd4j-cloud-observability` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-observability` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-observability` 给出实现选择、证据状态和剩余风险。”
