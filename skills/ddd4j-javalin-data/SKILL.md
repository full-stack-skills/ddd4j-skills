...
description: Use when integrating MyBatis, JPA, PostgreSQL, repositories, transactions, EntityManagerFactory, EventStore, Projection, or Outbox with ddd4j-javalin.
license: Apache-2.0
---
...
统一介绍 MyBatis、JPA/PostgreSQL、Repository、事务、EMF、EventStore、Projection、Outbox。先用 ddd4j-javalin-version-selection 确认维护线，再读取当前分支源码。

## 核心范围

MyBatis、JPA/PostgreSQL、Repository、事务、EMF、EventStore、Projection、Outbox。
...
- “使用 `$ddd4j-javalin-data` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-javalin-data` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-javalin-data` 给出实现选择、证据状态和剩余风险。”
