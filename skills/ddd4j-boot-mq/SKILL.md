...
统一介绍 Kafka、RabbitMQ、Pulsar、RocketMQ、ActiveMQ、MQTT、NATS、SQS、ONS/TDMQ、listener 生命周期，不按单个 artifact 拆技能。使用前先通过 ddd4j-boot-version-selection 确认维护线。

## 核心范围

Kafka、RabbitMQ、Pulsar、RocketMQ、ActiveMQ、MQTT、NATS、SQS、ONS/TDMQ、listener 生命周期。
...
- “使用 `$ddd4j-boot-mq` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-boot-mq` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-boot-mq` 给出实现选择、证据状态和剩余风险。”
