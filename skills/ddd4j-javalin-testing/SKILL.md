...
description: Use when testing ddd4j-javalin across Javalin 6 and 7 with HTTP contracts, Keycloak, PostgreSQL, MQ brokers, Testcontainers, Docker conditions, ports, and lifecycle tests.
license: Apache-2.0
---
...
统一介绍 Javalin 6/7 HTTP、Keycloak、PostgreSQL、MQ、Testcontainers、Docker、port 0、生命周期。先用 ddd4j-javalin-version-selection 确认维护线，再读取当前分支源码。

## 核心范围

Javalin 6/7 HTTP、Keycloak、PostgreSQL、MQ、Testcontainers、Docker、port 0、生命周期。
...
- “使用 `$ddd4j-javalin-testing` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-javalin-testing` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-javalin-testing` 给出实现选择、证据状态和剩余风险。”
