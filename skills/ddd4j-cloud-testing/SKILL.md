...
description: Use when testing ddd4j-cloud compatibility, Web MVC and MySQL, context propagation, Feign, Spring Cloud Stream binders, databases, brokers, or remote consumers.
license: Apache-2.0
---
...
统一介绍 compatibility、WebMVC/MySQL、Context、Feign、Binder、DB、Broker、remote consumer，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。
...
- “使用 `$ddd4j-cloud-testing` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-testing` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-testing` 给出实现选择、证据状态和剩余风险。”
