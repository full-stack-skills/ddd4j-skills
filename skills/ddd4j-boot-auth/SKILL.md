...
description: Use when choosing or integrating Sa-Token, Apache Shiro, or Spring Security through ddd4j-boot auto-configuration, Subject mapping, exception handling, and request lifecycle.
license: Apache-2.0
---
...
统一介绍 Sa-Token、Shiro、Spring Security、SubjectProvider、异常与请求清理，不按单个 artifact 拆技能。使用前先通过 ddd4j-boot-version-selection 确认维护线。

## 核心范围

Sa-Token、Shiro、Spring Security、SubjectProvider、异常与请求清理。
...
- “使用 `$ddd4j-boot-auth` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-boot-auth` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-boot-auth` 给出实现选择、证据状态和剩余风险。”
