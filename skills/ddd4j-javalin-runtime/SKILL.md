...
统一介绍 start、validate、initialize、SPI、readiness、drain、rollback、shutdown hook、close。先用 ddd4j-javalin-version-selection 确认维护线，再读取当前分支源码。
...
- “使用 `$ddd4j-javalin-runtime` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-javalin-runtime` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-javalin-runtime` 给出实现选择、证据状态和剩余风险。”
