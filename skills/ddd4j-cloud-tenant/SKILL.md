...
统一介绍 Tenant 注解、TenantContextHolder、系统隔离、数据范围、SQL、跨服务传播，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。

## 核心范围

Tenant 注解、TenantContextHolder、系统隔离、数据范围、SQL、跨服务传播。
...
- “使用 `$ddd4j-cloud-tenant` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-tenant` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-tenant` 给出实现选择、证据状态和剩余风险。”
