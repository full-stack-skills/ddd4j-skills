...
description: Use when integrating ddd4j MQ adapters with Quarkus, including Kafka, NATS, other brokers, acknowledgment, listeners, lifecycle, and Testcontainers.
license: Apache-2.0
---
...
统一介绍 Kafka、NATS、其他当前 Broker、ACK、listener、lifecycle、Testcontainers，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。

## 核心范围

Kafka、NATS、其他当前 Broker、ACK、listener、lifecycle、Testcontainers。
...
- “使用 `$ddd4j-quarkus-mq` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-mq` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-mq` 给出实现选择、证据状态和剩余风险。”
