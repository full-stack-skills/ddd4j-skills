...
description: Use when building ddd4j-cloud Spring MVC or WebFlux services with internationalized responses, errors, context, tenant, authentication, validation, and readiness.
license: Apache-2.0
---
...
统一介绍 MVC、WebFlux、i18n response、errors、context、tenant、auth、validation、readiness，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。
...
- “使用 `$ddd4j-cloud-web` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-web` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-web` 给出实现选择、证据状态和剩余风险。”
