...
统一介绍 Actuator、liveness/readiness、metrics、trace、logging、monitor、OpenTelemetry，不按单个 artifact 拆技能。使用前先通过 ddd4j-boot-version-selection 确认维护线。
...
- “使用 `$ddd4j-boot-observability` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-boot-observability` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-boot-observability` 给出实现选择、证据状态和剩余风险。”
