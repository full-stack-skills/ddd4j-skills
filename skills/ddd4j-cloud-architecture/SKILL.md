...
description: Use when designing or reviewing ddd4j-cloud parent, BOM, dependencies, extensions, compatibility tests, verification consumers, samples, and upstream Boot or ddd4j boundaries.
license: Apache-2.0
---
...
统一介绍 parent、bom、dependencies、extensions、compatibility-tests、verification、samples、上游边界，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。

## 核心范围

parent、bom、dependencies、extensions、compatibility-tests、verification、samples、上游边界。
...
- “使用 `$ddd4j-cloud-architecture` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-architecture` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-architecture` 给出实现选择、证据状态和剩余风险。”
