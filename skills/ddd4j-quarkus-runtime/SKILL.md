...
统一介绍 CDI/Arc、CommandBus、DomainEventPublisher、SubjectProvider、Context、startup/shutdown，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。
...
- “使用 `$ddd4j-quarkus-runtime` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-runtime` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-runtime` 给出实现选择、证据状态和剩余风险。”
