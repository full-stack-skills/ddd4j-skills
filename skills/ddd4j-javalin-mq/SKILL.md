...
description: Use when wiring ddd4j MQ clients and listeners into Javalin lifecycle, including required or optional consumers, acknowledgment, retry, readiness, rollback, and shutdown.
license: Apache-2.0
---
...
统一介绍 MQClient、listener、required/optional、ACK、retry、readiness、rollback、close。先用 ddd4j-javalin-version-selection 确认维护线，再读取当前分支源码。

## 核心范围

MQClient、listener、required/optional、ACK、retry、readiness、rollback、close。
...
- “使用 `$ddd4j-javalin-mq` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-javalin-mq` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-javalin-mq` 给出实现选择、证据状态和剩余风险。”
