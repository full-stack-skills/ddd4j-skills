...
description: Use when building ddd4j Quarkus REST endpoints with request context, errors, authentication, validation, idempotency, CORS, limits, and readiness.
license: Apache-2.0
---
...
统一介绍 Quarkus REST/JAX-RS、Context、错误、Auth、Validation、幂等、CORS、Readiness，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。

## 核心范围

Quarkus REST/JAX-RS、Context、错误、Auth、Validation、幂等、CORS、Readiness。
...
- “使用 `$ddd4j-quarkus-web` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-web` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-web` 给出实现选择、证据状态和剩余风险。”
