...
description: Use when creating or reviewing ddd4j Quarkus extensions with runtime and deployment modules, processors, BuildItems, recorders, configuration, bean registration, or native image support.
license: Apache-2.0
---
...
统一介绍 runtime/deployment、Processor、BuildItem、Recorder、配置、Bean 注册、native image，不按 artifact 拆技能。先选择 3.3.x 或 4.0.x，再读取当前源码。

## 核心范围

runtime/deployment、Processor、BuildItem、Recorder、配置、Bean 注册、native image。
...
- “使用 `$ddd4j-quarkus-extension-authoring` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-quarkus-extension-authoring` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-quarkus-extension-authoring` 给出实现选择、证据状态和剩余风险。”
