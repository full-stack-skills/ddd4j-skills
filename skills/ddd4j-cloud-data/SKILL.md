...
description: Use when integrating ddd4j-cloud data sources, MyBatis, JPA, transactions, Seata, tenants, migrations, repositories, or database-specific behavior.
license: Apache-2.0
---
...
统一介绍 datasource、MyBatis、JPA、transaction、Seata、tenant、migration、repository，不按单个 artifact 拆技能。先选 Cloud→Boot→ddd4j 主组合，再读取当前分支。
...
- “使用 `$ddd4j-cloud-data` 分析我当前项目应该采用的实现和配置。”
- “使用 `$ddd4j-cloud-data` 对照当前源码审查现有用法。”
- “使用 `$ddd4j-cloud-data` 给出实现选择、证据状态和剩余风险。”
