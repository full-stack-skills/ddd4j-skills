...
- [x] **Step 1:** 提取八条 Cloud→Boot→ddd4j→JDK/Maven 组合。
- [x] **Step 2:** 记录来源、SHA、主/备选和验证级别。
- [x] **Step 3:** 分类上游缺件与 Cloud 自身失败。
- [x] **Step 4:** 运行矩阵脚本、TRACE 并提交。

### Task 2: context、tenant、Feign

**Files:** Create skills/ddd4j-cloud-context/、skills/ddd4j-cloud-tenant/、skills/ddd4j-cloud-feign/

- [x] **Step 1:** 追踪过滤器→ThreadContext→异步→Feign 的传播与恢复。
- [x] **Step 2:** Tenant 覆盖表注解、系统隔离、数据范围和忽略条件。
- [x] **Step 3:** Feign 覆盖 header、error decoder、重试和清理。
- [x] **Step 4:** 用异常、线程复用、并发场景验证并提交。

### Task 3: data 与 stream

**Files:** Create skills/ddd4j-cloud-data/、skills/ddd4j-cloud-stream/

- [x] **Step 1:** Data 统一 datasource、MyBatis/JPA、事务和 Seata。
- [x] **Step 2:** Stream 解释 function/binding/physical destination 三层命名。
- [x] **Step 3:** 覆盖 Kafka、RabbitMQ、Pulsar、RocketMQ 的命名和 ACK/NACK。
- [x] **Step 4:** 使用真实 Broker round-trip 校正并提交。

### Task 4: web 与 observability

**Files:** Create skills/ddd4j-cloud-web/、skills/ddd4j-cloud-observability/

- [x] **Step 1:** Web 覆盖 MVC/WebFlux、i18n、异常和 context。
- [x] **Step 2:** Observability 覆盖 Nacos、Sentinel、monitor、trace、readiness。
- [x] **Step 3:** 区分配置、启动和真实外部服务验收。
- [x] **Step 4:** 场景验证后提交。
...
- [x] **Step 1:** Testing 覆盖 compatibility、WebMVC/MySQL、Binder 容器。
- [x] **Step 2:** Release 覆盖八线串行构建和私服发布。
- [x] **Step 3:** 隔离缓存消费 parent/BOM/JAR；上游缺件标 BLOCKED。
