...
- [x] **Step 1:** 从三分支 POM、Wrapper、workflow、BuildLineContractTest 提取版本。
- [x] **Step 2:** 固化 7.1.x Maven 3、7.2.x Maven 4 的证据。
- [x] **Step 3:** 绘制模块和请求生命周期。
- [x] **Step 4:** 用新项目、升级、维护三场景验证并提交。

### Task 2: runtime 与 web

**Files:** Create skills/ddd4j-javalin-runtime/、skills/ddd4j-javalin-web/

- [x] **Step 1:** Runtime 覆盖 start→validate→initialize→ready→drain→close。
- [x] **Step 2:** 覆盖部分失败反向回滚、hook 移除和幂等 close。
- [x] **Step 3:** Web 覆盖 route、Context、错误、CORS、幂等、request size、async。
- [x] **Step 4:** 分别验证 Javalin 6/7 HTTP contract 并提交。
...
- [x] **Step 1:** 建立三框架选择矩阵。
- [x] **Step 2:** 每框架说明依赖、配置、SubjectProvider、路由和错误。
- [x] **Step 3:** Keycloak 要求取 Token、合法 200、缺失/非法 401/403。
- [x] **Step 4:** 验证请求结束 Subject/Context 清理后提交。
...
- [x] **Step 1:** Data 统一 MyBatis/JPA/PostgreSQL、Repository、事务和 EMF。
- [x] **Step 2:** MQ 覆盖 required/optional、ACK、重试、死信和关闭。
- [x] **Step 3:** Cache 区分 Caffeine 与共享 CAS。
- [x] **Step 4:** Extensions 说明 Module override 和生命周期后提交。
...
- [x] **Step 1:** Testing 统一 HTTP、Keycloak、PostgreSQL、MQ 和 Docker 条件。
- [x] **Step 2:** Release 固化三线最终 SHA、CI、私服和空缓存消费。
- [x] **Step 3:** Production hardening 路由功能细节，保留计划审批、跨能力 TDD 和发布门禁。
- [x] **Step 4:** 全量链接、TRACE、插件和 README 验证后提交。
